...
const getKoiosHost = ({ isMainnet }: Config) =>
  isMainnet ? 'api.koios.rest' : 'testnet.koios.rest'
const createKoios = (config: Config) =>
  axios.create({ baseURL: `https://${getKoiosHost(config)}` })
...
const assetUTxOsQuery = async (
  ...
  switch (config.queryAPI.type) {
    ...
    case 'koios': {
      ...
      if (address) {
        const aiq = await koios.get('/api/v0/address_info', {
          ...
          result = {
            ...
            data: info.utxo_set
              ...
              ?.filter((info) =>
                info.assets?.find(
                  (asst) =>
                    asst.assetName == asset.name &&
                    asst.policyId == asset.policyId
                )
              ),
          }
      }
...
const addressUTxOsQuery = async (address: string, config: Config) => {
  ...
  switch (config.queryAPI.type) {
    ...
    case 'koios': {
      const koios = createKoios(config)
      if (address) {
        const adata = await koios.get('/api/v0/address_info', {
          params: { _address: address },
        })
        type Info = {
          balance: string
...
const protocolParametersQuery = async (config: Config) => {
  ...
  switch (config.queryAPI.type) {
    case 'graphql': {
      ...
      const qdata = await apollo.query<QueryData>({
      ...
      if (params)
        result = {
          ...
          data: {
            ...
            slot: qdata.data.cardano.tip.slotNo,
    ...
    case 'koios': {
      ...
      if (tip) {
        const epData = await koios.get('/api/v0/epoch_params', {
          params: { _epoch_no: tip.epoch },
        })
