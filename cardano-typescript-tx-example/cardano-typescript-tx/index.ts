...
export class CardanoWallet {
    ...
    async transaction({
        ...
        for (let recipient of recipients) {
            ...
            try {
                mintedAssets = burn ? null : this._makeMintedAsset(recipient?.mintedAssets || [])
            } catch (error) {
                throw Error
            }
            ...
            if (!burn && mintedAssets && (recipient.mintedAssets || []).length > 0) {
        ...
        const walletCollateral = await this.wallet?.experimental?.getCollateral()
        if (redeemers?.length >= 1) {
            RawTransaction = await _txBuilderSpendFromPlutusScript({
                ...
                collateral: walletCollateral ? [walletCollateral] : [],
                presetUtxos: presetUtxos
            })
        } else if (minting > 0) {
    ...
    async createLockingPolicyScript(address: string, expirationTime: Date | null, protocolParameters: ProtocolParameters) {
        ...
        const ttl = !expirationTime || !duration ? null : slot + duration;
