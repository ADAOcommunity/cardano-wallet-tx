...
export async function _txBuilderSpendFromPlutusScript({
  ...
  const transaction = Transaction.new(txBody, witnesses, aux)
...
export async function _txBuilder({
  ...
  txBuilder.add_change_if_needed(
    Address.from_bytes(Buffer.from(PaymentAddress, 'hex'))
  )
...
const createTxBuilder: (
  protocolParameters: ProtocolParameters
) => TransactionBuilder = (protocolParameters: ProtocolParameters) => {
  const {
    minFeeA,
    minFeeB,
    poolDeposit,
