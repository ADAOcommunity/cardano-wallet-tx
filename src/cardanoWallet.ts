...
import {
  _txBuilder,
  _txBuilderMinting,
  _txBuilderSpendFromPlutusScript,
} from './transactions'
import { ProtocolParameters, UTxO } from './queryApi'
import {
  MintedAsset,
  Policy,
  UTXO,
...
export class CardanoWallet {
  ...
  public async enable(walletname: string) {
    ...
    this._walletApi = await (walletConfig as any)[
  ...
  async utxoFromData(
    ...
    return this.lib.TransactionUnspentOutput.new(
      this.lib.TransactionInput.new(
        this.lib.TransactionHash.from_bytes(Buffer.from(output.txHash, 'hex')),
        output.index
      ),
  ...
  async assetsToValue(
    ...
    policies.forEach((policy) => {
      const policyAssets = assets.filter((asset) => asset.policyId === policy)
  ...
  async getBalance(
    ...
    if (!this.wallet || !protocolParameters)
      return {
        lovelace: '0',
        assets: [],
      }
    ...
    const minAda = this.lib.min_ada_required(
      countedValue,
      false,
      this.lib.BigNum.from_str(
        this._protocolParameter.coinsPerUtxoWord.toString()
      )
    )
  ...
  async getUtxos(utxosHex: string[]): Promise<UTXO[]> {
    ...
    for (const utxo of Utxos) {
      const assets = this._utxoToAssets(utxo)

      UTXOS.push({
        txHash: Buffer.from(utxo.input().transaction_id().to_bytes()).toString(
          'hex'
        ),
  ...
  async transaction({
    ...
    this._protocolParameter = ProtocolParameters
    if (!this._protocolParameter) return null
    const utxos = utxosRaw.map((u) => u)
    const mintedAssetsArray: MintedAsset[] = []
    const outputs = this.lib.TransactionOutputs.new()
    console.log('transaction')
    let minting = 0
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const costValues: any = {}
    for (const recipient of recipients) {
      const lovelace = Math.floor(
        (Number(recipient.amount) || 0) * 1000000
      ).toString()
      ...
      if (!burn && recipient?.mintedAssets) {
        try {
          mintedAssets = this._makeMintedAsset(recipient?.mintedAssets)
        } catch (error) {
      ...
      if (mintedAssets && !burn && (recipient.mintedAssets || []).length > 0) {
        ...
        const requiredMintAda = this.lib.Value.new(
      ...
      let addr
      try {
        addr = this.lib.Address.from_bytes(
      ...
      let outputBuilder =
        this.lib.TransactionOutputBuilder.new().with_address(addr)
      let output
      ...
      if (parseInt(outputValue.coin().to_str()) > 0) {
        if (multiasst) {
          ...
          output = outputBuilder.next().with_coin(outputValue.coin()).build()
        }
      } else if (multiasst) {
        output = outputBuilder
          .next()
    ...
    if (redeemers?.length >= 1 && this.wallet) {
  ...
  async createLockingPolicyScript(
    ...
    const ttl = !duration || !expirationTime ? null : slot + duration
  ...
  // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
  async signTx(transaction: string | Transaction, partialSign = false) {
    if (!this.wallet) return null
  ...
  async signData(data: string) {
    const address = await this.getBaseAddress()
    if (!address || !this.wallet) return null
    const coseSign1Hex = await this.wallet.signData(
      address,
  ...
  hashMetadata(metadata: { [s: string]: unknown } | ArrayLike<unknown>) {
    const aux = this.lib.AuxiliaryData.new()
  ...
  async submitTx(
    ...
    const transaction = this.lib.Transaction.from_bytes(
